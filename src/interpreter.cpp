...
void Interpreter::trace(const std::string& msg) {
    if (debugMode_) *stderr_ << "[TRACE] " << msg << "\n";
}
...
void Interpreter::notifyWatchers(const std::string& varName, ValuePtr newVal) {
    ...
    while (env) {
        for (auto& w : env->watches()) {
            ...
            auto savedEnv = currentEnv_;
            currentEnv_ = w.scope;
            try { execStmt(*static_cast<Stmt*>(w.body)); } catch (ReturnSignal&) {}
            currentEnv_ = savedEnv;
        }
        env = env->parent().get();
    }
...
void Interpreter::execStmt(const Stmt& s) {
    std::visit([&](const auto& node) {
        ...
        if constexpr (std::is_same_v<T, VarDeclStmt>)       execVarDecl(node);
        else if constexpr (std::is_same_v<T, FnDeclStmt>)   execFnDecl(node);
        else if constexpr (std::is_same_v<T, ClassDeclStmt>) execClassDecl(node);
        else if constexpr (std::is_same_v<T, StructDeclStmt>)execStructDecl(node);
        else if constexpr (std::is_same_v<T, InterfaceDeclStmt>) execInterfaceDecl(node);
        else if constexpr (std::is_same_v<T, BlockStmt>)    execBlock(node);
        ...
        else if constexpr (std::is_same_v<T, ReturnStmt>)   execReturn(node);
        ...
        else if constexpr (std::is_same_v<T, ExprStmt>)     evalExpr(*node.expr);
        else if constexpr (std::is_same_v<T, StreamOutStmt>)execStreamOut(node);
        else if constexpr (std::is_same_v<T, WatchStmt>)    execWatch(node);
        else if constexpr (std::is_same_v<T, SignalDeclStmt>)execSignalDecl(node);
        else if constexpr (std::is_same_v<T, EmitStmt>)     execEmit(node);
        else if constexpr (std::is_same_v<T, OnStmt>)       execOn(node);
        else if constexpr (std::is_same_v<T, ImportStmt>)   execImport(node);
        else if constexpr (std::is_same_v<T, UnsafeStmt>)   execUnsafe(node);
        else if constexpr (std::is_same_v<T, DeferStmt>)    execDefer(node);
    }, s.data);
}
...
void Interpreter::execBlock(const BlockStmt& b, std::shared_ptr<Environment> env) {
    ...
    deferStack_.push_back({});
    try {
        for (auto& stmt : b.stmts) execStmt(*stmt);
    } catch (...) {
        // Run deferred blocks even on exceptions
        for (auto it = deferStack_.back().rbegin(); it != deferStack_.back().rend(); ++it)
            try { execStmt(**it); } catch (...) {}
        deferStack_.pop_back();
        currentEnv_ = savedEnv;
        throw;
    }
...
void Interpreter::execVarDecl(const VarDeclStmt& s) {
    ...
    if (s.initializer) val = evalExpr(*s.initializer);
...
void Interpreter::execClassDecl(const ClassDeclStmt& s) {
    ...
    for (auto& m : s.members) {
        if (auto* fn = std::get_if<FnDeclStmt>(&m->data)) {
            ...
            if (vd->initializer) init = evalExpr(*vd->initializer);
...
void Interpreter::execIf(const IfStmt& s) {
    ...
    if (cond->truthy()) execStmt(*s.thenBranch);
    else if (s.elseBranch) execStmt(*s.elseBranch);
}
...
void Interpreter::execFor(const ForStmt& s) {
    ...
    auto doBody = [&](ValuePtr item) {
        ...
        try { execStmt(*s.body); }
        catch (BreakSignal&)    { popEnv(); return false; }
    ...
    if (iterable->isList()) {
        for (auto& elem : iterable->asList()->elements)
            if (!doBody(elem)) break;
    } else if (iterable->isStr()) {
        for (char c : iterable->asStr())
            if (!doBody(makeStr(std::string(1,c)))) break;
        ...
        throw RuntimeError("Value of type '" + iterable->typeName() + "' is not iterable", s.line);
    }
}

void Interpreter::execReturn(const ReturnStmt& s) {
    ValuePtr val = makeNull();
    if (s.value) val = evalExpr(*s.value);
...
void Interpreter::execEmit(const EmitStmt& s) {
    ...
    if (!sig) throw RuntimeError("Signal '" + s.name + "' is not declared", s.line);
    for (auto& [body, scope] : sig->handlers) {
        auto savedEnv = currentEnv_;
        currentEnv_ = scope;
        try { execStmt(*static_cast<Stmt*>(body)); }
        catch (ReturnSignal&) {}
        currentEnv_ = savedEnv;
    }
}
...
ValuePtr Interpreter::evalExpr(const Expr& e) {
    return std::visit([&](const auto& node) -> ValuePtr {
        using T = std::decay_t<decltype(node)>;
        if constexpr (std::is_same_v<T, IntLitExpr>)    return evalIntLit(node);
        if constexpr (std::is_same_v<T, FloatLitExpr>)  return evalFloatLit(node);
        if constexpr (std::is_same_v<T, BoolLitExpr>)   return evalBoolLit(node);
        if constexpr (std::is_same_v<T, NullLitExpr>)   return evalNullLit(node);
        if constexpr (std::is_same_v<T, StringLitExpr>) return evalStringLit(node);
        if constexpr (std::is_same_v<T, CharLitExpr>)   return evalCharLit(node);
        if constexpr (std::is_same_v<T, PSStringExpr>)  return evalPSString(node);
        if constexpr (std::is_same_v<T, IdentExpr>)     return evalIdent(node);
        if constexpr (std::is_same_v<T, BinaryExpr>)    return evalBinary(node);
        if constexpr (std::is_same_v<T, UnaryExpr>)     return evalUnary(node);
        if constexpr (std::is_same_v<T, AssignExpr>)    return evalAssign(node);
        if constexpr (std::is_same_v<T, CallExpr>)      return evalCall(node);
        if constexpr (std::is_same_v<T, IndexExpr>)     return evalIndex(node);
        if constexpr (std::is_same_v<T, MemberExpr>)    return evalMember(node);
        if constexpr (std::is_same_v<T, PipelineExpr>)  return evalPipeline(node);
        if constexpr (std::is_same_v<T, NullCoalExpr>)  return evalNullCoal(node);
        if constexpr (std::is_same_v<T, ListLitExpr>)   return evalListLit(node);
        if constexpr (std::is_same_v<T, DictLitExpr>)   return evalDictLit(node);
        if constexpr (std::is_same_v<T, NewExpr>)       return evalNew(node);
        if constexpr (std::is_same_v<T, TernaryExpr>)   return evalTernary(node);
...
ValuePtr Interpreter::evalIntLit(const IntLitExpr& e)   { return makeInt(e.value); }
ValuePtr Interpreter::evalFloatLit(const FloatLitExpr& e){ return makeFloat(e.value); }
ValuePtr Interpreter::evalBoolLit(const BoolLitExpr& e)  { return makeBool(e.value); }
ValuePtr Interpreter::evalNullLit(const NullLitExpr&)    { return makeNull(); }
ValuePtr Interpreter::evalStringLit(const StringLitExpr& e){ return makeStr(e.value); }
ValuePtr Interpreter::evalCharLit(const CharLitExpr& e)  { return makeStr(std::string(1, e.value)); }
...
ValuePtr Interpreter::evalBinary(const BinaryExpr& e) {
    ...
    if (e.op == "&&") {
        ...
        if (!l->truthy()) return makeBool(false);
        return makeBool(evalExpr(*e.right)->truthy());
    }
    if (e.op == "||") {
        auto l = evalExpr(*e.left);
        if (l->truthy()) return makeBool(true);
        return makeBool(evalExpr(*e.right)->truthy());
    }
    ...
    // Bitwise
    if (e.op == "&") return makeInt(l->asInt() & r->asInt());
    if (e.op == "|") return makeInt(l->asInt() | r->asInt());
    if (e.op == "^") return makeInt(l->asInt() ^ r->asInt());
...
ValuePtr Interpreter::applyBinaryArith(const std::string& op, ValuePtr l, ValuePtr r, int line) {
    ...
    if (op == "+") {
        ...
        if (l->isList() && r->isList()) {
            ...
            for (auto& e : l->asList()->elements) res->elements.push_back(e);
            for (auto& e : r->asList()->elements) res->elements.push_back(e);
            return makeList(res);
        }
    }
    ...
    if (useFloat) {
        ...
        if (op == "+") return makeFloat(a + b);
        if (op == "-") return makeFloat(a - b);
        if (op == "*") return makeFloat(a * b);
        if (op == "/") {
            if (b == 0.0) throw RuntimeError("Division by zero", line);
            return makeFloat(a / b);
        }
        if (op == "%") return makeFloat(std::fmod(a, b));
        if (op == "**") return makeFloat(std::pow(a, b));
    } else {
        int64_t a = l->asInt(), b = r->asInt();
        if (op == "+") return makeInt(a + b);
        if (op == "-") return makeInt(a - b);
        if (op == "*") return makeInt(a * b);
        if (op == "/") {
            if (b == 0) throw RuntimeError("Division by zero", line);
            return makeInt(a / b);
        }
        if (op == "%") {
            if (b == 0) throw RuntimeError("Modulo by zero", line);
            return makeInt(a % b);
        }
        if (op == "**") return makeFloat(std::pow((double)a, (double)b));
    }
    throw RuntimeError("Cannot apply '" + op + "' to types " + l->typeName() + " and " + r->typeName(), line);
}

ValuePtr Interpreter::applyBinaryCompare(const std::string& op, ValuePtr l, ValuePtr r, int line) {
    if (op == "==") return makeBool(l->equals(*r));
    if (op == "!=") return makeBool(!l->equals(*r));

    auto cmp = [&]() -> int {
        if (l->isInt() && r->isInt())    return (l->asInt() < r->asInt()) ? -1 : (l->asInt() > r->asInt()) ? 1 : 0;
        if ((l->isInt()||l->isFloat()) && (r->isInt()||r->isFloat())) {
            double a = l->asFloat(), b = r->asFloat();
            return (a < b) ? -1 : (a > b) ? 1 : 0;
        }
        if (l->isStr() && r->isStr()) return l->asStr().compare(r->asStr());
        throw TypeError("Cannot compare " + l->typeName() + " and " + r->typeName(), line);
    };

    int c = cmp();
    if (op == "<")  return makeBool(c < 0);
    if (op == ">")  return makeBool(c > 0);
    if (op == "<=") return makeBool(c <= 0);
    if (op == ">=") return makeBool(c >= 0);
...
ValuePtr Interpreter::evalUnary(const UnaryExpr& e) {
    ...
    if (e.op == "!") return makeBool(!val->truthy());
    if (e.op == "-") {
...
ValuePtr Interpreter::evalAssign(const AssignExpr& e) {
    ...
    if (auto* id = std::get_if<IdentExpr>(&e.target->data)) {
        ...
        if (reactive) notifyWatchers(id->name, newVal);
    ...
    if (callee->isClassDef()) {
        ...
        for (auto& [n, method] : def->ctorOrder) {
            if (inst->members->has(method)) {
                ...
                if (mv->isFn()) callMethod(inst, method, args, e.line);
            }
        }
        // If there's an 'init' or 'constructor' method, call it
        if (inst->members->has("init")) callMethod(inst, "init", args, e.line);
        else if (inst->members->has("constructor")) callMethod(inst, "constructor", args, e.line);
...
ValuePtr Interpreter::callFunction(std::shared_ptr<FunctionValue> fn,
    ...
    if (!fn->body) throw RuntimeError("Function '" + fn->name + "' has no body", line);
...
ValuePtr Interpreter::evalIndex(const IndexExpr& e) {
    ...
    if (obj->isList()) {
        ...
        if (i < 0) i += elems.size();
        if (i < 0 || (size_t)i >= elems.size())
            throw RuntimeError("List index " + std::to_string(key->asInt()) + " out of bounds", e.line);
        return elems[i];
    }
    if (obj->isDict()) {
...
ValuePtr Interpreter::evalMember(const MemberExpr& e) {
    ...
    if (obj->isNull() && e.safe) return makeNull();
    ...
    if (obj->isClassInst()) {
        ...
        // Return bound method
        throw RuntimeError("Member '" + e.member + "' not found on " + inst->def->name, e.line);
    }
    ...
    if (obj->isStructInst()) {
        ...
        throw RuntimeError("Field '" + e.member + "' not found on struct " + inst->def->name, e.line);
    }
...
ValuePtr Interpreter::evalPipeline(const PipelineExpr& e) {
    ...
    if (auto* call = std::get_if<CallExpr>(&e.right->data)) {
        std::vector<ValuePtr> args;
        args.push_back(left);
        for (auto& a : call->args) args.push_back(evalExpr(*a));
        auto callee = evalExpr(*call->callee);
        if (callee->isFn()) return callFunction(callee->asFn(), std::move(args), e.line);
        // Method call on left?
        if (auto* mem = std::get_if<MemberExpr>(&call->callee->data)) {
            auto obj = evalExpr(*mem->object);
            // Builtin method
            std::vector<ValuePtr> mArgs;
            for (auto& a : call->args) mArgs.push_back(evalExpr(*a));
    ...
    if (auto* id = std::get_if<IdentExpr>(&e.right->data)) {
        ...
        if (fn->isFn()) return callFunction(fn->asFn(), {left}, e.line);
...
ValuePtr Interpreter::evalListLit(const ListLitExpr& e) {
    ...
    for (auto& elem : e.elements) lv->elements.push_back(evalExpr(*elem));
    return makeList(lv);
}
...
ValuePtr Interpreter::evalNew(const NewExpr& e) {
    ...
    if (classDef->isStructDef()) {
        ...
        inst->def = def;
        for (size_t i = 0; i < def->fields.size(); i++) {
            inst->fields[def->fields[i].first] = (i < args.size()) ? args[i] : makeNull();
        }
        return makeStructInst(inst);
    }
...
ValuePtr Interpreter::callBuiltinMethod(ValuePtr obj, const std::string& method,
    ...
    if (obj->isStr()) {
        const std::string& s = obj->asStr();
        if (method == "length" || method == "len")  return makeInt(s.size());
        if (method == "upper") {
            std::string r = s;
        ...
        if (method == "trim") {
            ...
            return (l == std::string::npos) ? makeStr("") : makeStr(s.substr(l, r2-l+1));
        }
        if (method == "contains") {
            if (args.empty()) throw RuntimeError("contains() requires an argument", line);
            return makeBool(s.find(args[0]->toString()) != std::string::npos);
        }
        if (method == "startsWith") {
            if (args.empty()) throw RuntimeError("startsWith() requires an argument", line);
            std::string p = args[0]->toString();
            return makeBool(s.substr(0, p.size()) == p);
        }
        if (method == "endsWith") {
            if (args.empty()) throw RuntimeError("endsWith() requires an argument", line);
            std::string p = args[0]->toString();
            return makeBool(s.size() >= p.size() && s.substr(s.size()-p.size()) == p);
        }
        if (method == "split") {
        ...
        if (method == "replace") {
            if (args.size() < 2) throw RuntimeError("replace() requires 2 arguments", line);
        ...
        if (method == "slice" || method == "substr") {
            ...
            if (start < 0) start = std::max((int64_t)0, (int64_t)s.size() + start);
            if (end < 0)   end   = std::max((int64_t)0, (int64_t)s.size() + end);
            start = std::min(start, (int64_t)s.size());
            end   = std::min(end,   (int64_t)s.size());
            return makeStr(s.substr(start, end-start));
        }
        if (method == "toString" || method == "str") return obj;
        if (method == "toInt")   return makeInt(std::stoll(s));
        if (method == "toFloat") return makeFloat(std::stod(s));
        if (method == "repeat") {
        ...
        if (method == "chars") {
            auto lv = std::make_shared<ListValue>();
            for (char c : s) lv->elements.push_back(makeStr(std::string(1,c)));
            return makeList(lv);
        }
    }
    ...
    if (obj->isList()) {
        ...
        if (method == "push" || method == "add" || method == "append") {
            if (!args.empty()) lv->elements.push_back(args[0]);
            return makeNull();
        }
        if (method == "pop") {
            if (lv->elements.empty()) return makeNull();
        ...
        if (method == "unshift") {
            if (!args.empty()) lv->elements.insert(lv->elements.begin(), args[0]);
            return makeNull();
        }
        if (method == "contains" || method == "includes") {
            if (args.empty()) return makeBool(false);
            for (auto& e : lv->elements)
                if (e->equals(*args[0])) return makeBool(true);
            return makeBool(false);
        }
        if (method == "reverse") {
        ...
        if (method == "slice") {
            int64_t start = args.empty() ? 0 : args[0]->asInt();
            int64_t end = (args.size()>1) ? args[1]->asInt() : (int64_t)lv->elements.size();
            if (start < 0) start += lv->elements.size();
            if (end < 0)   end   += lv->elements.size();
        ...
        if (method == "map") {
            if (args.empty() || !args[0]->isFn()) throw RuntimeError("map() requires a function", line);
            auto nl = std::make_shared<ListValue>();
            for (auto& e : lv->elements)
        ...
        if (method == "filter") {
            if (args.empty() || !args[0]->isFn()) throw RuntimeError("filter() requires a function", line);
            auto nl = std::make_shared<ListValue>();
            for (auto& e : lv->elements) {
                auto r = callFunction(args[0]->asFn(), {e}, line);
                if (r->truthy()) nl->elements.push_back(e);
            }
            return makeList(nl);
        }
        if (method == "reduce") {
            if (args.empty() || !args[0]->isFn()) throw RuntimeError("reduce() requires a function", line);
            if (lv->elements.empty()) return args.size() > 1 ? args[1] : makeNull();
        ...
        if (method == "first") return lv->elements.empty() ? makeNull() : lv->elements.front();
        if (method == "last")  return lv->elements.empty() ? makeNull() : lv->elements.back();
        if (method == "clear") { lv->elements.clear(); return makeNull(); }
        if (method == "toString") return makeStr(obj->toString());
    }
    ...
    if (obj->isDict()) {
        ...
        if (method == "keys") {
            auto lv = std::make_shared<ListValue>();
            for (auto& [k,v] : dv->pairs) lv->elements.push_back(k);
            return makeList(lv);
        }
        if (method == "values") {
            auto lv = std::make_shared<ListValue>();
            for (auto& [k,v] : dv->pairs) lv->elements.push_back(v);
            return makeList(lv);
        }
        ...
        if (method == "remove") {
            if (!args.empty()) {
                ...
                dv->pairs.erase(std::remove_if(dv->pairs.begin(), dv->pairs.end(),
                    [&](auto& p){ return p.first->isStr() && p.first->asStr() == key; }),
                    dv->pairs.end());
            }
            return makeNull();
        }
    ...
    if (method == "withProperties") return obj;
...
void Interpreter::registerBuiltins() {
    ...
    defNative("int", [](std::vector<ValuePtr> args) -> ValuePtr {
        ...
        auto& v = args[0];
        if (v->isInt())   return v;
        if (v->isFloat()) return makeInt((int64_t)v->asFloat());
        if (v->isStr()) {
            try { return makeInt(std::stoll(v->asStr())); } catch (...) {}
        }
        return makeInt(0);
    });
    defNative("float", [](std::vector<ValuePtr> args) -> ValuePtr {
        if (args.empty()) return makeFloat(0.0);
        auto& v = args[0];
        if (v->isFloat()) return v;
        if (v->isInt())   return makeFloat((double)v->asInt());
        if (v->isStr()) {
            try { return makeFloat(std::stod(v->asStr())); } catch (...) {}
        }
    ...
    defNative("len", [](std::vector<ValuePtr> args) -> ValuePtr {
        if (args.empty()) return makeInt(0);
        auto& v = args[0];
        if (v->isStr())  return makeInt(v->asStr().size());
        if (v->isList()) return makeInt(v->asList()->elements.size());
        if (v->isDict()) return makeInt(v->asDict()->pairs.size());
        if (v->isSet())  return makeInt(v->asSet()->elements.size());
        return makeInt(0);
    });
    ...
    defNative("range", [](std::vector<ValuePtr> args) -> ValuePtr {
        ...
        if (args.size() == 1) { end = args[0]->asInt(); }
        else if (args.size() >= 2) { start=args[0]->asInt(); end=args[1]->asInt(); }
        if (args.size() >= 3) step = args[2]->asInt();
        if (step == 0) step = 1;
        auto lv = std::make_shared<ListValue>();
        if (step > 0) for (int64_t i=start; i<end; i+=step) lv->elements.push_back(makeInt(i));
        else          for (int64_t i=start; i>end; i+=step) lv->elements.push_back(makeInt(i));
        return makeList(lv);
    });
    ...
    defNative("sqrt",  [](std::vector<ValuePtr> a)->ValuePtr{ return makeFloat(std::sqrt(a.empty()?0:a[0]->asFloat())); });
    defNative("pow",   [](std::vector<ValuePtr> a)->ValuePtr{ return makeFloat(std::pow(a.size()<2?1:a[0]->asFloat(), a.size()<2?1:a[1]->asFloat())); });
    defNative("floor", [](std::vector<ValuePtr> a)->ValuePtr{ return makeInt((int64_t)std::floor(a.empty()?0:a[0]->asFloat())); });
    defNative("ceil",  [](std::vector<ValuePtr> a)->ValuePtr{ return makeInt((int64_t)std::ceil(a.empty()?0:a[0]->asFloat())); });
    defNative("round", [](std::vector<ValuePtr> a)->ValuePtr{ return makeInt((int64_t)std::round(a.empty()?0:a[0]->asFloat())); });
    defNative("max",   [](std::vector<ValuePtr> a)->ValuePtr{
        if(a.empty()) return makeNull();
        ValuePtr m = a[0];
        for(auto& v:a) if(v->asFloat()>m->asFloat()) m=v;
        return m;
    });
    ...
    defNative("sin",   [](std::vector<ValuePtr> a)->ValuePtr{ return makeFloat(std::sin(a.empty()?0:a[0]->asFloat())); });
    defNative("cos",   [](std::vector<ValuePtr> a)->ValuePtr{ return makeFloat(std::cos(a.empty()?0:a[0]->asFloat())); });
    defNative("tan",   [](std::vector<ValuePtr> a)->ValuePtr{ return makeFloat(std::tan(a.empty()?0:a[0]->asFloat())); });
    defNative("log",   [](std::vector<ValuePtr> a)->ValuePtr{ return makeFloat(std::log(a.empty()?1:a[0]->asFloat())); });
    defNative("log2",  [](std::vector<ValuePtr> a)->ValuePtr{ return makeFloat(std::log2(a.empty()?1:a[0]->asFloat())); });
    defNative("log10", [](std::vector<ValuePtr> a)->ValuePtr{ return makeFloat(std::log10(a.empty()?1:a[0]->asFloat())); });
    ...
    globalEnv_->define("INF",   makeFloat(std::numeric_limits<double>::infinity()), false);
    globalEnv_->define("NEG_INF",makeFloat(-std::numeric_limits<double>::infinity()), false);
    globalEnv_->define("NAN",   makeFloat(std::numeric_limits<double>::quiet_NaN()), false);
    ...
    for (auto& p : {"PROP_FROZEN","PROP_READONLY","PROP_IMMUTABLE","PROP_REACTIVE",
                    "PROP_SERIALIZABLE","PROP_UNSAFE","PROP_VOLATILE","PROP_DEBUG",
                    "PROP_HIDDEN","PROP_PRIVATE","PROP_PUBLIC","PROP_FINAL",
                    "PROP_CONST","PROP_STATIC","PROP_SYNC","PROP_ASYNC",
                    "PROP_LAZY","PROP_CACHED","PROP_TEMP","PROP_NATIVE",
                    "PROP_PROTECTED","PROP_INTERNAL","PROP_EXPERIMENTAL",
                    "PROP_DEPRECATED","PROP_LOCKED","PROP_OBSERVABLE"}) {
    ...
    defNative("ord", [](std::vector<ValuePtr> args)->ValuePtr{
        if(args.empty()||!args[0]->isStr()||args[0]->asStr().empty()) return makeInt(0);
