...
ValuePtr DictValue::get(const std::string& key) const {
    for (auto& p : pairs) {
        if (p.first->isStr() && p.first->asStr() == key) return p.second;
    }
    return nullptr;
}
...
int64_t Value::asInt() const {
    ...
    if (isFloat()) return static_cast<int64_t>(std::get<double>(data));
...
bool Value::truthy() const {
    return std::visit([](const auto& v) -> bool {
        ...
        if constexpr (std::is_same_v<T, double>)  return v != 0.0 && !std::isnan(v);
        if constexpr (std::is_same_v<T, std::string>) return !v.empty();
        if constexpr (std::is_same_v<T, char>) return v != '\0';
...
std::string Value::toString() const {
    return std::visit([](const auto& v) -> std::string {
        ...
        if constexpr (std::is_same_v<T, std::shared_ptr<StructInstance>>) {
            if (!v || !v->def) return "<struct instance>";
...
std::string Value::typeName() const {
    return std::visit([](const auto& v) -> std::string {
        ...
        if constexpr (std::is_same_v<T, int64_t>) return "int_64";
        if constexpr (std::is_same_v<T, double>)  return "float_64";
        if constexpr (std::is_same_v<T, std::string>) return "str";
        if constexpr (std::is_same_v<T, char>)    return "char";
        if constexpr (std::is_same_v<T, std::shared_ptr<FunctionValue>>) return "fn";
        if constexpr (std::is_same_v<T, std::shared_ptr<ClassDef>>)     return "class";
        if constexpr (std::is_same_v<T, std::shared_ptr<ClassInstance>>) return "instance";
        if constexpr (std::is_same_v<T, std::shared_ptr<StructDef>>)    return "struct";
        if constexpr (std::is_same_v<T, std::shared_ptr<StructInstance>>)return "struct_instance";
        if constexpr (std::is_same_v<T, std::shared_ptr<ListValue>>)    return "list";
        if constexpr (std::is_same_v<T, std::shared_ptr<SetValue>>)     return "set";
        if constexpr (std::is_same_v<T, std::shared_ptr<DictValue>>)    return "dict";
        if constexpr (std::is_same_v<T, std::shared_ptr<PtrValue>>)     return "ptr";
        return "unknown";
    }, data);
}
